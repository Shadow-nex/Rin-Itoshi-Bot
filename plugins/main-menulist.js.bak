...
└────────────`;
    ...
    const imagenBuffer = await (await fetch(imgUrl)).arrayBuffer();
    const thumb2 = Buffer.from(imagenBuffer);

    const imgenUrl = 'https://tinyurl.com/2c2wh7v2';
    const imgBuffer = await (await fetch(imgenUrl)).arrayBuffer();
    const thumb = Buffer.from(imgBuffer);

    const docBuffer = thumb2;
